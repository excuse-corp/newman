...
class CompressRouteTests(unittest.TestCase):
    ...
    def test_get_session_prefers_latest_context_usage_record(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(context_usage["auto_compact_limit"], 86808)
            self.assertEqual(context_usage["soft_compact_limit"], 73786)
            ...
            self.assertAlmostEqual(context_usage["budget_pressure"], 800 / 86808)
    ...
    def test_context_usage_falls_back_to_assembled_prompt_estimate_without_confirmed_usage(self) -> None:
        ...
        self.assertEqual(snapshot.soft_compact_limit, 73786)
