...
实现四层记忆模型（System / User / Session / Long-term）和上下文压缩机制，确保"文件即记忆"原则。
...
- Checkpoint 生成、存储、恢复
- 上下文压缩策略（80% / 92% 双阈值）
- MEMORY.md 读取与全量注入（MVP）
- Long-term Memory 向量化召回（P1）
...
memory/
  ...
  memory_extract.py       # Long-term Memory 提取
...
| Long-term Memory | MEMORY.md | 跨会话重要信息，MVP 全量注入 |
...
- Stable Context 与 Long-term Memory 不参与裁剪

### Checkpoint 设计要求

- 可读：用户可直接查看 checkpoint JSON
- 可替换：支持手动编辑
- 可恢复：从 checkpoint 恢复会话上下文
- 可递归：支持持续递归压缩
...
3. Checkpoint 可恢复且可读
4. 会话历史 JSON 可用户直接查看/编辑
5. 压缩后的摘要保留关键信息（人工抽样验证）
