...
- 不同类型节点要有一致但可区分的视觉样式
- 主区节点主文案必须是用户可理解的进展描述
- `tool / skill / agent / raw event` 只能作为次级标签、状态或 Drawer 内容

### 呈现分层

- 主层：一句用户可理解的执行进展，例如“正在查看相关文件”
- 次层：低调展示技术元信息，例如工具名、Skill 名、耗时、状态、结果数量
- 详情层：在右侧 Drawer 中展示原始输入输出、引用、错误细节和结构化字段

### 节点更新规则

- 同一个 `tool_call_id` 的 `started / finished / retry / error` 优先更新同一节点，不在主区连续堆叠多条日志
- 同一个审批请求的 `request / resolved` 优先更新同一节点
- 只有用户视角上“发生了新的阶段变化”时，才新增主区节点
...
前端必须采用“原始事件 -> 归一化节点 -> 主区文案”的固定流程：

1. 接收 SSE 原始事件
2. 归一化为内部 `timeline item`
3. 按事件类型和结构化字段选择文案模板
4. 把工具名、Skill 名、Agent 名、耗时、状态等作为次级元信息附着到节点
5. 把原始输入输出和引用证据保留到 Drawer
...
| --- | --- | --- | --- |
| `assistant_delta` | `assistant_stream` | 否 | 直接进入回答流，不作为过程节点 |
...
| `final_response` | `final_answer` | 是 | 最终结论卡片 |
...
| `final_answer` | completed | 不走模板，直接显示最终回答内容 | 引用数、来源数 |
...
| 工具类别 | 命中规则示例 | 主区运行中文案模板 | 主区完成文案模板 |
| --- | --- | --- | --- |
| 文件查看 | `read_file` | `正在查看文件` | `文件已查看完毕` |
| 目录浏览 | `list_dir` | `正在查看目录结构` | `目录结构已确认` |
| 文件检索 | `search_files` | `正在检索相关文件` | `已找到 {count} 个相关文件` |
| 网页抓取 | `fetch_url` | `正在查看网页资料` | `网页资料已获取` |
| 知识库检索 | `search_knowledge_base` | `正在检索知识资料` | `已找到 {count} 条知识结果` |
| 计划更新 | `update_plan` | `先列一下执行步骤` | `执行步骤已更新` |
| 代码或文件修改 | `edit_file` 等写操作 | `准备修改相关内容` | `相关内容已更新` |
| 命令执行 / 测试 / 构建 | `terminal`、`run`、`test`、`build` | `正在运行检查` | `检查已完成` |
| 提取 / 摘要 / 比对 | `extract`、`summarize`、`diff`、`compare` | `正在整理关键信息` | `关键信息已整理完成` |
| 外部能力调用 | `mcp__*` | `正在调用外部能力` | `外部能力调用已完成` |
| 未知工具 | fallback | `正在获取完成任务所需信息` | `这一步已完成` |
...
- Context Usage 显示可用
- 移动端不崩布局
- 主区过程文案不直接暴露原始事件名，默认以用户可理解进展呈现
- 主区过程文案由确定性映射逻辑生成，不依赖 LLM 重写
